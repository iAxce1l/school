...
<body>
    ...
    <body>
        <div class="form">
            ...
            <p>Nachádzate sa na stránke admina.</p>
        ...
        <div class="row m-4 justify-content-center align-items-center text-center">
            ...
            if (!$conn) {
                die("Zle pripojenie :( " . mysqli_connect_error());
            }

            $sql = "SELECT * FROM udaje";
            $udaje = mysqli_query($conn, $sql);
            echo "<div class='tablička'>";
            echo "<table style='background-color:black; border: 1px solid white;'>";
            echo "<tr><th style='border: 1px solid white;'>Meno</th><th style='border: 1px solid white;'>Priezvisko</th><th style='border: 1px solid white;'>Email</th><th style='border: 1px solid white;'>Správa</th><th style='border: 1px solid white;'>ID</th></tr>";
            if (mysqli_num_rows($udaje) > 0) {
                while ($row = mysqli_fetch_assoc($udaje)) {
                    echo "<tr><td style='border: 1px solid white;'>" . $row["meno"] . "</td><td style='border: 1px solid white;'>" . $row["priezvisko"] . "</td><td style='border: 1px solid white;'>" . $row["email"] . "</td><td style='border: 1px solid white;'>" . $row["sprava"] . "</td><td style='border: 1px solid white;'>" . $row["id"] . "</td></tr>";
                }
            } else {
                echo "<tr><td colspan='5' style='border: 1px solid white;'>Nič sa nenašlo :(.</td></tr>";
            }
            echo "</table>";
            echo "</div>";
            ?>
